...
class _ReportsAnalyticsScreenState extends State<ReportsAnalyticsScreen>
  ...
  // --- LOAD USER DATA FIRST, THEN FETCH ---
  Future<void> _loadUserDataAndFetch() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      _userType = prefs.getString('userType') ?? 'admin';
      _userId = prefs.getString('user_id') ?? prefs.getInt('userId')?.toString() ?? '1';
      _companyId = prefs.getString('company_id') ?? '1';
  ...
  Future<void> _fetchAllData() async {
    ...
    try {
      ...
      // Get all raw data
      List<dynamic> allClients = clientsData['data'] ?? [];
      List<dynamic> allProjects = projectsData['data'] ?? [];
      List<dynamic> allEmployees = employeesData['data'] ?? [];
      List<dynamic> allInvoices = invoicesData['data'] ?? [];
      List<dynamic> allBugs = bugsData['data'] ?? [];
      ...
      print('Total Bugs: ${allBugs.length}');
      print('=======================');

      // ==========================================
      // 🔥 ADMIN vs CLIENT DATA FILTERING
      // ==========================================
      if (_userType == 'admin') {
        // ✅ ADMIN: Filter by company_id ONLY
        _clients = allClients.where((c) {
          final cId = c['company_id']?.toString();
          return cId == _companyId;
        }).toList();

        _projects = allProjects.where((p) {
          final cId = p['company_id']?.toString();
          return cId == _companyId;
        }).toList();

        _employees = allEmployees.where((e) {
          final cId = e['company_id']?.toString();
          return cId == _companyId;
        }).toList();

        _invoices = allInvoices.where((i) {
          final cId = i['company_id']?.toString();
          return cId == _companyId;
        }).toList();

        _bugs = allBugs.where((b) {
          final cId = b['company_id']?.toString();
          return cId == _companyId;
        }).toList();

        print('=== ADMIN FILTERED DATA ===');
        print('Clients for company $_companyId: ${_clients.length}');
        print('Projects for company $_companyId: ${_projects.length}');
        print('Employees for company $_companyId: ${_employees.length}');
        print('Invoices for company $_companyId: ${_invoices.length}');
        print('Bugs for company $_companyId: ${_bugs.length}');
        print('============================');

      } else if (_userType == 'client') {
        // ✅ CLIENT: Filter by client_id ONLY
        _clients = allClients.where((c) {
          final clientId = c['client_id']?.toString() ?? '';
          final createdByClient = c['created_by_client']?.toString() ?? '';
          return clientId == _userId || createdByClient == _userId;
        }).toList();

        _projects = allProjects.where((p) {
          final clientId = p['client_id']?.toString() ?? '';
          final createdByClient = p['created_by_client']?.toString() ?? '';
          return clientId == _userId || createdByClient == _userId;
        }).toList();

        _employees = allEmployees.where((e) {
          final clientId = e['client_id']?.toString() ?? '';
          final createdByClient = e['created_by_client']?.toString() ?? '';
          return clientId == _userId || createdByClient == _userId;
        }).toList();

        _invoices = allInvoices.where((i) {
          final clientId = i['client_id']?.toString() ?? '';
          final createdByClient = i['created_by_client']?.toString() ?? '';
          return clientId == _userId || createdByClient == _userId;
        }).toList();

        _bugs = allBugs.where((b) {
          final clientId = b['client_id']?.toString() ?? '';
          final createdByClient = b['created_by_client']?.toString() ?? '';
          return clientId == _userId || createdByClient == _userId;
        }).toList();

        print('=== CLIENT FILTERED DATA ===');
        print('Clients for client $_userId: ${_clients.length}');
        print('Projects for client $_userId: ${_projects.length}');
        print('Employees for client $_userId: ${_employees.length}');
        print('Invoices for client $_userId: ${_invoices.length}');
        print('Bugs for client $_userId: ${_bugs.length}');
        print('============================');
        ...
        _bugs = allBugs;
        print('=== FALLBACK: SHOWING ALL DATA ===');
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            const Text("Reports & Analytics", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            const SizedBox(width: 8),
            // 🔥 Show user type badge
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
              decoration: BoxDecoration(
                color: _userType == 'admin' ? Colors.blue.withOpacity(0.3) : Colors.green.withOpacity(0.3),
    ...
    if (_filteredProjects.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.code, size: 48, color: AppColors.textMuted),
            const SizedBox(height: 16),
            Text(
              _userType == 'admin'
          ...
          children: [
            ...
            Text(
              _userType == 'admin'
                  ...
                  : "No employees found for your account",
              style: const TextStyle(color: AppColors.textMuted),
            ),
          ],
        ),
